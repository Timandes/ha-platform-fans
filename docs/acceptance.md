...
设计基线为 2026-09-14，最终本机验证日期为 2026-09-15。候选初次交付状态为**模拟通过**；已有其他研究中的 NUC9 只读与交叉 PWM 证据不能替代本项目候选镜像的实机验收。初次交付阶段没有传输源码/镜像至 NAS，没有真实硬件映射、部署或写入，也没有修改 EFI、BIOS 字段、thermal trip 或通知参数。

## 用户授权后的在线实机进展

2026-09-15后续已按用户要求合并/推送main，并在NAS-11检出、导入同一候选完成在线实机验收。发现并修复PCI读取所需SYS_ADMIN；完成41档PWM、80/80、当前Fixed40恢复、180秒自动曲线、s6保活及受控源故障。原BIOS自动策略、长测及主机重启仍pending。具体版本、统计、限制与原始证据见[实机报告](acceptance-nas11-20260915.md)。下方软件结果及pending表是初次交付记录，当前实机状态以该报告为准。
