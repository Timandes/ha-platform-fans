...
- 自动升速按每路输入独立等待：CPU风扇的CPU输入1秒；SYS的CPU及每块NVMe输入各5秒。需求必须持续高于该风扇当前已确认PWM，回落或相等就重新计时，源之间不能共用等待时间。降速仍等待10秒。显式boost阈值（CPU90°C、NVMe53°C）及必需热源故障立即全速，不受升速等待限制。启动/切换策略/成功配置更新按现有规则立即提交初始目标并清空等待计时。必需热源缺失或过期时请求两组上限100%并退出，s6负责重启；CPU运行下限仍40%，SYS下限30%。
